...
import { capitalize, getDisplayName, getModelUrl } from '~/utils/string-helpers';
...
/**
 * The allowlist is checked only on `Fee`. A tip's description is written by the
 * tipper — `tipUser` spreads it into the row — so a type-agnostic check would let
 * one buy a system-voice line on any surface rendering through here.
 */
