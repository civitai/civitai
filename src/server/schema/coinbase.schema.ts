...
export const createBuzzChargeSchema = z.object({
  ...
  // and the check passes — structurally, for every non-multiple of ten below 2^53, not at some
  // sampled rate. (The bound is MAGNITUDE, not representability; see `buzz-charge.ts`.) This
  // line is the only thing on this route that rejects the fraction.
