...
/**
 * PREVIEW half of the environment-scoped cache-key prefixing coverage (production half is
 ...
 *
 * Pins that on a preview deployment every cache key and every `tag:<name>` set moves into the
 * preview namespace, that a tag bust still finds its members there, and that a production entry
 * sitting under the unprefixed key is neither read nor overwritten. Expected keys are
 * hand-written literals; `hashifyObject` is stubbed to a constant.
 */
...
describe('cache-helpers key prefixing — preview', () => {
  ...
  it('exposes the preview prefix', async () => {
  ...
  it('writes queryCache entries and tag sets under prefixed keys', async () => {
    ...
    // The production keys must be untouched by a preview write.
