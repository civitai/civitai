 ...
 *
 * The introspection caller (`civitai-link-service`) is deliberately NOT registered by a migration:
 * its secret is the credential verifier and cannot be committed to a public repo, so the row is
 * created per environment through the hub's own client UI. See docs/features/civitai-link.md.
 */
...
describe('the introspection caller is not registered by any migration', () => {
  it('no migration writes a civitai-link-service client row', () => {
    const offenders = fs
      .readdirSync(MIGRATIONS, { withFileTypes: true })
      .filter((e) => e.isDirectory())
      .filter((e) => {
        const file = path.join(MIGRATIONS, e.name, 'migration.sql');
        return (
          fs.existsSync(file) && fs.readFileSync(file, 'utf8').includes('civitai-link-service')
        );
      })
      .map((e) => e.name);
    ...
    expect(
      offenders,
      'the introspection client is created per environment through the client UI, because its ' +
        'secret is a credential verifier and this repo is public — see docs/features/civitai-link.md'
    ).toEqual([]);
  });
});
