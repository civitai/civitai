    ...
    return {
      ...
      modelVersions: filteredVersions.map((version) => {
        ...
        return {
          ...
          metrics: undefined,
          rank: {
            generationCount: versionMetrics?.generationCount ?? 0,
            downloadCount: versionMetrics?.downloadCount ?? 0,
            ratingCount: versionMetrics?.ratingCount ?? 0,
            rating: Number(versionMetrics?.rating?.toFixed(2) ?? 0),
            thumbsUpCount: versionMetrics?.thumbsUpCount ?? 0,
            thumbsDownCount: versionMetrics?.thumbsDownCount ?? 0,
            earnedAmount: versionMetrics?.earnedAmount ?? 0,
          },
