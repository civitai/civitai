...
vi.mock('~/server/services/user-hub.service', () => ({
  ...
  getHubSourceCandidates: vi.fn().mockResolvedValue({ name: 'a', sources: [], total: 0 }),
  getHubSourceGroups: vi.fn().mockResolvedValue([]),
  ...
  searchHubSources: vi.fn().mockResolvedValue([]),
...
const requiredScopes: Record<string, number> = {
  ...
  searchSources: TokenScope.UserRead,
  resolveSource: TokenScope.UserRead,
  upsert: TokenScope.UserWrite,
  sourceCandidates: TokenScope.UserRead,
  sourceGroups: TokenScope.UserRead,
