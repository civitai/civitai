...
export const productBadgeRouter = router({
  getProductsWithBadges: moderatorProcedure
    .input(getProductsWithBadgesInput)
    .query(({ input }) => {
      return getProductsWithBadges(input);
    }),
  getBadgeHistory: moderatorProcedure
    .input(getBadgeHistoryInput)
    .query(({ input }) => {
      return getBadgeHistory(input);
    }),
  upsertProductBadge: moderatorProcedure
    .input(upsertProductBadgeInput)
    .mutation(({ input }) => {
