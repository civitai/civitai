  ...
  >`
    SELECT
        ...
        (SELECT COUNT(*)::INT FROM "Image" i
          ...
          WHERE i."ingestion" = 'Scanned' AND i."needsReview" IS NULL AND i."hidden" IS NULL AND i."userId" = u.id AND i."postId" IS NOT NULL AND i."type" = 'image'::"MediaType"
        ) as "imageCount",
        (SELECT COUNT(*)::INT FROM "Image" i
          INNER JOIN "Post" p ON p."id" = i."postId" AND p."publishedAt" IS NOT NULL AND p.availability != 'Private'
          WHERE i."ingestion" = 'Scanned' AND i."needsReview" IS NULL AND i."hidden" IS NULL AND i."userId" = u.id AND i."postId" IS NOT NULL AND i."type" = 'video'::"MediaType"
        ) as "videoCount",
