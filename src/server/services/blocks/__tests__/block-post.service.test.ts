...
describe('resolveGalleryTarget', () => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  /**
   * 🔴 THE COLLABORATOR GUARD — the publisher is not the only declared party.
   *
   * Seats live on the app's STORE LISTING, so this reaches them via
   * `appBlockId`; `appId` cannot get there. Every case below keeps the version
   * fixture valid and the publisher DIFFERENT from the model owner, so the five
   * availability checks and the publisher check all pass and cannot take credit
   * for any refusal here — and every refusal is asserted on its OWN message,
   * which is the only thing that separates it from the publisher refusal sitting
   * one line above it.
   */
  describe('🔴 THE COLLABORATOR GUARD — an accepted seat is a declared party too', () => {
    /** The listing behind the calling app block. Distinct from every other id. */
    const LISTING_ID = 'lst_alpha';
    ...
    beforeEach(() => {
      ...
      dbMock.dbRead.appBlock.findUnique.mockResolvedValue({
        id: ACTOR.appBlockId,
        app: { userId: PUBLISHER_USER_ID },
        appListing: { id: LISTING_ID },
      });
      // No seat, unless a case says otherwise.
      dbMock.dbRead.appCollaborator.findFirst.mockResolvedValue(null);
    });

    it('REFUSES a model whose owner holds an ACCEPTED seat on the app’s listing', async () => {
      dbMock.dbRead.appCollaborator.findFirst.mockResolvedValue({ userId: OTHER_OWNER });

      await expectRejection(
        call(),
        'FORBIDDEN',
        'this app may not attach posts to its collaborators’ models'
      );
    });

    it('ALLOWS the identical request when there is NO seat', async () => {
      // The positive control. Without it, a mutant refusing EVERY gallery attach
      // would pass the case above.
      await expect(call()).resolves.toMatchObject({ modelVersionId: 3100 });
    });

    it('asks about the MODEL OWNER, not the poster and not the publisher', async () => {
      // The subject of this question is a third party. Asking it about the caller
      // would type-check, would compile, and would be a completely different
      // control — so the argument is pinned rather than inferred.
      await call();

      const where = dbMock.dbRead.appCollaborator.findFirst.mock.calls[0][0].where;
      expect(where.userId).toBe(OTHER_OWNER);
      expect(where.userId).not.toBe(VIEWER_USER_ID);
      expect(where.userId).not.toBe(PUBLISHER_USER_ID);
    });

    it('keys the seat lookup on the LISTING id, and filters on ACCEPTED', async () => {
      // Asserted on the QUERY because the mock is free to ignore a `where` — a
      // fixture-based test of either property would be a claim about the mock.
      //
      // Both halves are load-bearing. A seat call keyed on `appBlockId` compiles
      // (both are strings) and silently matches NOTHING, so the guard would be
      // permanently inert with nothing failing. And admitting a PENDING invite
      // would turn the guard into a griefing lever: a listing owner could make a
      // rival's gallery unreachable through the app by inviting them.
      await call();

      const where = dbMock.dbRead.appCollaborator.findFirst.mock.calls[0][0].where;
      expect(where.appListingId).toBe(LISTING_ID);
      expect(where.status).toBe('accepted');
    });

    it('ALLOWS when the app block has no store listing — there is nobody to be a party', async () => {
      // An AppBlock without a listing is an ordinary state. Failing closed here
      // would refuse every gallery attach for such an app, to fix nothing.
      dbMock.dbRead.appBlock.findUnique.mockResolvedValue({
        id: ACTOR.appBlockId,
        app: { userId: PUBLISHER_USER_ID },
        appListing: null,
    ...
    it('does NOT ask about seats at all when the PUBLISHER check already refused', async () => {
      // Ordering claim, and the discriminator that keeps the two refusals
      // separable: a publisher-owned target must refuse with the publisher's own
      // message and cost no seat lookup.
      dbMock.dbRead.modelVersion.findUnique.mockResolvedValue(
        version({ model: { userId: PUBLISHER_USER_ID } })
      );
    ...
    it('a seat lookup FAILURE throws rather than reading as "not affiliated"', async () => {
      // Fail direction. `safeCollaboratorQuery` swallows ONLY a missing-table
      // error; anything else must propagate, or an unavailable database silently
      // answers "no seat" and the guard is off exactly when it cannot be checked.
      dbMock.dbRead.appCollaborator.findFirst.mockRejectedValue(new Error('connection reset'));
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * 🔴 `assertGalleryTargetMatchesSources` — the attach must not CONTRADICT the
 * images.
 *
 * Nothing else on this path relates `modelVersionId` to `sources[]`: every other
 * gallery check asks whether the TARGET is legal, none asks whether it has
 * anything to do with what is being posted.
 *
 * ⚠️ THE GUARD REFUSES A CONTRADICTION AND DOES NOT REQUIRE A PROOF, and these
 * cases are written to pin BOTH halves. The three allow-cases below are not
 * oversights being papered over — each is a case where the server has no evidence,
 * and pinning them is what stops a later "tightening" turning the guard into one
 * that refuses honest posts. They are the asymmetry, asserted.
 */
describe('assertGalleryTargetMatchesSources', () => {
  const TARGET_VERSION = 3100;
  const UNRELATED_VERSION = 4200;
  ...
  function workflowImage(modelVersionIds: number[]) {
    return {
  ...
  it('REFUSES a target none of the workflows used', () => {
    expect(() =>
      assertGalleryTargetMatchesSources({
        gallery: GALLERY,
        images: [workflowImage([UNRELATED_VERSION])],
      })
    ).toThrow('gallery target was not used by the images being posted');
  });

  it('ALLOWS a target the workflow did use', () => {
    // The positive control: identical except the id. Without it a mutant
    // refusing EVERY attach would pass the case above.
    expect(() =>
      assertGalleryTargetMatchesSources({
        gallery: GALLERY,
        images: [workflowImage([TARGET_VERSION])],
      })
    ).not.toThrow();
  });

  it('ALLOWS when the target is among SEVERAL resources — checkpoint or LoRA, either counts', () => {
    expect(() =>
      assertGalleryTargetMatchesSources({
        gallery: GALLERY,
        images: [workflowImage([UNRELATED_VERSION, TARGET_VERSION])],
      })
    ).not.toThrow();
  });

  it('unions across MULTIPLE workflow sources — one match anywhere is enough', () => {
    expect(() =>
      assertGalleryTargetMatchesSources({
        gallery: GALLERY,
        images: [workflowImage([UNRELATED_VERSION]), workflowImage([TARGET_VERSION])],
      })
    ).not.toThrow();
  });

  it('refuses a SIBLING VERSION of the same model — the gallery is per VERSION', () => {
    // `modelId` is deliberately the SAME as the gallery's; only the version
    // differs. A mutant comparing modelId, or comparing nothing, survives every
    // other case in this describe and dies here.
    expect(() =>
      assertGalleryTargetMatchesSources({
        gallery: GALLERY,
        images: [workflowImage([TARGET_VERSION + 1])],
      })
    ).toThrow('gallery target was not used by the images being posted');
  });

  describe('⚠️ the three cases it deliberately does NOT decide', () => {
    it('ALLOWS when any source is `published` — those rows carry no provenance at all', () => {
      // An app-published Image is created with `meta` NULL, no ImageResource rows
      // and no workflow link, so there is nothing to compare. Publish-then-post is
      // a supported flow; refusing it would break it outright.
      //
      // 🔴 THIS IS ALSO THE BYPASS, and it is pinned rather than hidden: one
      // published image disables the check for the whole post. Closing it needs
      // provenance written at PUBLISH time, not a stricter predicate here.
      expect(() =>
        assertGalleryTargetMatchesSources({
          gallery: GALLERY,
          images: [workflowImage([UNRELATED_VERSION]), publishedImage()],
        })
      ).not.toThrow();
    });

    it('ALLOWS when NO workflow had a readable resource record', () => {
      // Absence of evidence, not evidence of absence — the orchestrator has
      // several step shapes and the record is written by the submit path.
      expect(() =>
        assertGalleryTargetMatchesSources({ gallery: GALLERY, images: [workflowImage([])] })
      ).not.toThrow();
    });

    it('ALLOWS when only SOME workflows had a readable record — ANY gap, not ALL', () => {
      // 🔴 THE FALSE-REFUSAL CASE, and the reason the predicate is ANY rather
      // than ALL. Grading against the readable half alone would refuse an attach
      // to a model the UNREADABLE workflow genuinely used — the server simply
      // cannot see it. Note the fixture: the readable workflow names a DIFFERENT
      // version, so an `ALL`-shaped implementation has a non-empty set that
      // disagrees with the target and throws. This case dies under that mutant
      // and passes under the correct one.
      expect(() =>
        assertGalleryTargetMatchesSources({
          gallery: GALLERY,
          images: [workflowImage([UNRELATED_VERSION]), workflowImage([])],
        })
      ).not.toThrow();
    });

    it('ALLOWS when there is no gallery target at all', () => {
      expect(() =>
        assertGalleryTargetMatchesSources({
          gallery: null,
          images: [workflowImage([UNRELATED_VERSION])],
