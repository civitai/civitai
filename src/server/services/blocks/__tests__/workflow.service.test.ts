...
// an EDIT-ONLY checkpoint version and omitting `sourceImage` used to resolve to
// `txt2img` — and the graph did not reject it: it silently rewrote `model.id` to
// the same-index sibling in the txt2img version list and returned success. The
// caller was billed and got images generated by a checkpoint it never asked for,
// with nothing in the response or the `block_workflows` read-model revealing the
// swap.
//
...
describe('edit-only checkpoint version + no sourceImage', () => {
  // Qwen model 2268063 hosts BOTH: 2558804 is "Image Edit 2511" (img2img:edit
  // only), 2552908 is v2512 (txt2img only). Same model, disjoint workflows.
  const QWEN_EDIT_V2511 = 2558804;
  const QWEN_TXT_V2512 = 2552908;
  // Boogu: Edit / Edit Turbo are img2img:edit-only; Base / Turbo are txt2img-only.
  const BOOGU_EDIT = 3049824;
  const BOOGU_BASE = 3049541;
  // MageFlow: Standard(edit) vs Standard(txt2img).
  ...
  //
  // Characterization test, run against the REAL graph: this is what used to
  // happen — and still happens to anything that reaches the graph with this
  // shape. If the graph ever starts rejecting instead of substituting, this
  // fails and tells us the guard's premise changed.
  it('documents that the graph SILENTLY SUBSTITUTES an out-of-workflow version', () => {
    const externalCtx: GenerationCtx = {
      limits: { maxQuantity: 4, maxResources: 10, vidQuantity: 1 },
      user: { isMember: false, tier: 'free' },
      flags: {},
    ...
    expect((result.data as { model: { id: number } }).model.id).toBe(QWEN_TXT_V2512);
  });
