...
export const createFlux3VideoInput = defineHandler<LooseGenerationData, [VideoGenStepTemplate]>(
  (data) => {
    const startImage = data.images?.[0]?.url;
    const endImage = data.images?.[1]?.url;
    const draft = (data as { draft?: boolean }).draft;
    ...
    const shared = {
      ...
      resolution: draft
        ? '720p'
        : ((data as { resolution?: string }).resolution as Flux3V3TextToVideoInput['resolution']),
