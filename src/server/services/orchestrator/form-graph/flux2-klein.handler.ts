...
export const createFlux2KleinInput = defineHandler<LooseGenerationData, [ImageGenStepTemplate]>(
  (data, ctx) => {
    ...
    const cfgScale = isDistilled ? defaults.cfgScale : data.cfgScale ?? defaults.cfgScale;
    ...
    return [
      {
        ...
        input: removeEmpty({
          ...
          schedule: (data as { scheduler?: string }).scheduler ?? 'simple',
