...
export const createQwenInput = defineHandler<LooseGenerationData, [ImageGenStepTemplate]>(
  (data, ctx) => {
    ...
    if (data.ecosystem === 'Qwen3') {
      const baseInput = {
        ...
        promptExtend: (data as { enablePromptExpansion?: boolean }).enablePromptExpansion,
        quantity,
        seed: data.seed,
      };
