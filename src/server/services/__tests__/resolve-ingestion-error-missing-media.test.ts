 ...
 * therefore FAILS on the assertion rather than passing by coincidence.
 */
...
beforeEach(() => {
   ...
   * wrong reason.
   *
   * 🔴 But `resetAllMocks` is WRONG here, though it is right in the spoke's copy of this suite where
   * every mock in the file is the file's own. This suite runs under the global `setup.ts`, which
   * registers canonical hybrid mocks (`dbMock`, `loggingMock`) whose DEFAULT implementations are
   * installed once per file. `resetAllMocks` strips those too, so `logToAxiom(...)` returns
   * `undefined` and the production `.catch(() => undefined)` on it throws `Cannot read properties
   * of undefined`. So: clear globally (call history, which is what the assertions read), and RESET
   * this file's own mocks.
   */
  vi.clearAllMocks();
...
describe('the guard reports what it did, in both directions', () => {
  /**
   * Both hooks survive a revert unless something asserts them. They are the only signal
   * distinguishing a healthy run from two silent failure modes that look identical to it:
  ...
  it('logs a refusal when the bucket answered absent, naming WHICH refusal', async () => {
    ...
    // `presence` is carried rather than implied, so the line stays legible if a second refusing
    // verdict is ever added — a fail-CLOSED bucket misconfiguration can only ever produce `absent`.
  ...
  it('COUNTS the short-circuit at this call site, rather than passing silently', async () => {
     ...
     * a guard lies for months, so the caller is expected to log here".
     *
  ...
  it('does NOT log an inconclusive probe for a url that was never a key', async () => {
    /**
     * 🔴 The finding this fixes. A not-a-key short-circuit that returned `unknown` would make every
     * profile-picture url in the queue emit the same "could not consult the bucket" warning as a
     * genuine store outage — with an identical empty `error`. The one number this channel exists to
     * produce could not then answer its own question.
     */
    dbMock.dbRead.image.findUnique.mockResolvedValue({
      ingestion: 'Error',
      postId: null,
...
describe('the harness itself', () => {
  /**
   * 🔴 This suite's per-case reset, pinned — because the defect it prevents is LATENT and therefore
   * has no behavioural test that could fail. `vi.clearAllMocks()` clears CALLS and leaves
   * IMPLEMENTATIONS installed, so `headObject`'s `mockResolvedValue` from an earlier case would
   * survive into every later one. Nothing here currently depends on that, which is exactly why it
   * needs a structural guard rather than a behavioural one: the first case that forgets to arm the
   * store would pass for the wrong reason, and pass silently.
   *
