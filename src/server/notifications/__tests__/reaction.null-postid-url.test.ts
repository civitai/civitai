 ...
 * Fixing only the emitter leaves every link already sitting in a user's
 * notification history broken forever, because those rows cannot be rewritten.
 * Fixing only the page leaves the app still minting `?postId=null` links. Each
 * test below fails on its own half being reverted.
 */

type Def = (typeof reactionNotifications)['image-reaction-milestone'];
