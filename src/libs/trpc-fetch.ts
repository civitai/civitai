...
import type { TRPCResponse, TRPCResultResponse, TRPCSuccessResponse } from '@trpc/server/rpc';
...
const createTinyRPCClient = <TRouter extends AnyRouter>(baseUrl: string) =>
  createRecursiveProxy(async (opts) => {
    ...
    const stringifiedInput = input !== undefined && JSON.stringify({ json: input });
...
export const trpcFetch = createTinyRPCClient<AppRouter>('/api/trpc');
