...
// RichTextEditor reads the sticker flag and the owned-sticker list
// unconditionally, so mounting it pulls in FeatureFlagsCtx and, through
// useQueryUserCosmetics, CivitaiSessionContext — neither of which this scaffold
// provides. `sticker` is not in CONTROLS, so stub both to their empty state
// rather than standing up a session for a test about markdown import.
vi.mock('~/providers/FeatureFlagsProvider', () => ({
  useFeatureFlags: () => ({ stickers: false }),
}));
vi.mock('~/components/Sticker/sticker.util', async (importOriginal) => ({
  ...(await importOriginal<Record<string, unknown>>()),
  useOwnedSticker: () => ({ sticker: [], isLoading: false }),
}));
// The balances query is `enabled: false` here, but the hook still resolves tRPC
// context at render, so an unmocked trpc throws before the flag can matter.
vi.mock('~/utils/trpc', async (importOriginal) => ({
  ...(await importOriginal<Record<string, unknown>>()),
  trpc: { cosmetic: { getStickerBalances: { useQuery: () => ({ data: undefined }) } } },
}));
