...
import { CollectionType, EntityType } from '@prisma/client';
import {
  IconAlertTriangle,
  IconBolt,
  IconBookmark,
...
export function ImageDetailByProps({
  ...
  imageId: number;
  onClose: () => void;
  onNavigate: (image: ImagesInfiniteModel, index: number) => void;
  connect?: ConnectProps;
}) {
  const theme = useMantineTheme();
  const currentUser = useCurrentUser();
  const browserRouter = useBrowserRouter();
  const shareUrl = browserRouter.asPath;

  const { active = false } = browserRouter.query as { active?: boolean };
  const { data: fetchedImage, isInitialLoading } = trpc.image.get.useQuery(
    { id: imageId },
    { enabled: !imagesFromProps?.length }
  );

  const images = !!imagesFromProps?.length
    ? imagesFromProps
    : [fetchedImage as ImagesInfiniteModel | undefined].filter(isDefined);
    ...
    <>
      ...
      <div className="relative flex size-full max-h-full max-w-full overflow-hidden bg-gray-2 dark:bg-dark-9">
        <div className="relative flex flex-1 flex-col @max-md:pb-[60px]">
          <ImageGuard2 image={image} explain={false} {...connect}>
            {() => (
              <>
                {/* HEADER */}
                <div className="flex justify-between gap-8 p-3">
                  <CloseButton onClick={onClose} variant="filled" className="size-9 rounded-full" />
