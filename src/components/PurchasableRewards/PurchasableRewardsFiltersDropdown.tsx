...
type Filters = Omit<GetPaginatedPurchasableRewardsSchema, 'limit'>;

export function PurchasableRewardsFiltersDropdown({ filters, setFilters, ...buttonProps }: Props) {
  const { classes, theme, cx } = useStyles();
  const mobile = useIsMobile();

  const [opened, setOpened] = useState(false);
  const filterLength = !!filters.maxPrice ? 1 : 0;
