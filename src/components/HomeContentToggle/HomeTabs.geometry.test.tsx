...
describe('sub nav tab row geometry', () => {
  ...
  test('keeps the More button from being squashed by that shrink', async () => {
    ...
    // AWAITED, unlike the trailing ones: `tabRow()` resolves by document selector, so if teardown
    // has not finished when the second render measures, both arms read the FIRST render's row —
    // which was never under shrink pressure — and the comparison below is 28 against 28.
    await cleanup();
    ...
    expect(row.clientWidth).toBeLessThan(row.scrollWidth);
    expect(moreButton().getBoundingClientRect().width).toBeCloseTo(relaxed, 2);
    cleanup();
  });
