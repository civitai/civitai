...
/**
 * Handler-level coverage for POST /api/v1/blocks/shared-storage/unvote.
 *
 ...
 * `unvoteSharedRow` and is pinned in `apps-shared.router.test.ts`, including a
...
function createMocks({
  ...
  authorization = 'Bearer tok_unvote',
}: { method?: string; body?: unknown; authorization?: string } = {}) {
  const req = {
    method,
    body,
    query: {},
    url: '/api/v1/blocks/shared-storage/unvote',
...
const { mockUnvote, mockHandleEndpointError } = vi.hoisted(() => ({
  mockUnvote: vi.fn(),
  mockHandleEndpointError: vi.fn(),
}));
vi.mock('~/server/routers/apps-shared.router', () => ({
  unvoteSharedRow: mockUnvote,
  SHARED_KEY_MAX: 64,
}));
...
import handler from '~/pages/api/v1/blocks/shared-storage/unvote';
...
beforeEach(() => {
  ...
  mockUnvote.mockResolvedValue({ count: COUNT });
});

describe('POST /api/v1/blocks/shared-storage/unvote', () => {
  ...
  it('200: hands the BEARER token + the key down and returns the new tally', async () => {
    ...
    expect(mockUnvote).toHaveBeenCalledWith('tok_unvote', KEY);
  ...
  it('400 for an over-long key — the bound bites at 65, not 64', async () => {
    ...
    expect(mockUnvote).toHaveBeenCalledWith('tok_unvote', ok);
  });
  ...
  it('a failure goes to the shared error chokepoint, not a body built here', async () => {
    ...
    mockUnvote.mockRejectedValueOnce(boom);
    const { req, res } = createMocks({ body: { key: KEY } });
    await handler(req as never, res as never);
