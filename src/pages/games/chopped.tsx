...
function ChoppedContent() {
  const gameStatus = useChoppedStore((state) => state.game?.status);

  if (!gameStatus) return <Landing />;
  if (gameStatus === 'setup') return <Setup />;
  return <div>Chopped {gameStatus}</div>;
}
...
function Setup() {
  ...
  const [newGameState, setNewGameState] = useState<NewGame>({
    themeIds: shuffle(global.themes.map((theme) => theme.id)).slice(0, 2),
  ...
  return (
    <Stack>
      ...
      <Button size="lg" mt="md" disabled={!canStart}>
        Create Game
      </Button>
    </Stack>
  );
