 ...
 *
 * Split out so a test can import the matchers without importing a stdin handler that exits the
 * process on EOF — under vitest that takes the worker with it, which reads as a vanished suite
 * rather than a failure. The alternative, a main-module self-check, was measured to disable the
 * ENTIRE hook whenever the invocation path went through a symlink or a directory junction:
 * `resolve(process.argv[1])` is not a realpath and `import.meta.url` is, so the two disagree and
 * every Bash command runs unguarded, silently.
 *
 * Original header:
...
export const FULL_SUITE_REASON =
...
// Subcommands that MANAGE the package rather than run it. Without this the flag-value group eats
// the verb, so `pnpm -r add tsc` looked like `pnpm <flag> tsc` and was denied — installing tsc is
// not running it.
const PACKAGE_SUBCOMMANDS =
  '(?:add|remove|rm|uninstall|install|i|up|update|upgrade|why|list|ls|outdated|link|unlink|audit|dedupe|test|run)\\b';
...
const TSC_INVOCATION = new RegExp(
  String.raw`^\s*(?:\w+=\S*\s+)*(?:` +
    ...
    String.raw`(?:\s+-{1,2}[\w-]+(?:=\S+)?(?:\s+(?!${PACKAGE_SUBCOMMANDS}|tsc\b)[^-\s]\S*)?)*` +
...
// pnpm's SET selectors. `--filter '!@civitai/ui'` is every package except one, `...pkg` is a package
...
function isWorkspaceTarget(target) {
  ...
  if (target.kind === 'flag') {
    return !ROOT_TARGET.test(target.value) && !MULTI_PACKAGE_FILTER.test(target.value);
  }
...
export function directRootTypecheck(command) {
  ...
  return segments.some((seg, i) => {
    ...
    // A project inside a workspace package is the narrow run this guard makes room for.
    if (WORKSPACE_PATH.test(inline)) return false;
    // Everything else is the root program, including by another path and including a DIRECTORY:
    // tsc resolves `<dir>/tsconfig.json`, so dropping the filename was a one-token bypass —
    // `-p ../other-worktree` checked exactly what `-p ../other-worktree/tsconfig.json` does, and
    // only the second was denied. The one narrow spelling left is a project file with another
    // name, `-p tsconfig.scripts.json`, which is what the scripts gate itself recommends.
    return !/[^\\/]+\.json$/.test(inline) || /(?:^|[\\/])tsconfig\.json$/.test(inline);
  });
}

export const DIRECT_TSC_REASON =
