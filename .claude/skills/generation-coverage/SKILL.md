...
`GenerationCoverageNext` is a plain view, and it is the one the app reads — the Prisma `GenerationCoverage` model `@@map`s to it. Its first branch covers any version listed in `"EcosystemCheckpoints"`, and it applies **no status check**. So:
...
| --- | --- |
| `status --version <id>` | Whether the version has a row, and whether `GenerationCoverageNext` reports it covered. |
