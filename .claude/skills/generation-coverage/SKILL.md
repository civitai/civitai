...
`GenerationCoverage` is a plain view and the one the app reads. It carries both coverage rules as columns — `covered` (live) and `coveredNext` (staged) — and the Flipt boolean `generation-coverage-next` decides which one answers, globally, default off. Its `"EcosystemCheckpoints"` branch is shared by both rules and applies **no status check**. So:
...
- **The row overrides everything else — except moderation.** The version stays covered even after it is unpublished, but a top-level `m.mode IS NULL` means an `Archived` or `TakenDown` model is covered by no branch at all. For an `ExternalGeneration` version, branch 2 covers it once published, so removing the row after launch is what lets a later unpublish remove coverage. So far every row has been kept.
...
| --- | --- |
| `status --version <id>` | Whether the version has an `EcosystemCheckpoints` row, and what `GenerationCoverage` reports for **both** `covered` and `coveredNext`. |
