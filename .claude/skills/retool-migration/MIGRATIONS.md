...
| Workflows (2) | `868kn80u9` | 2 SQL | — | alerting, not a page | **not started** — and **not cron**: `crontab` is `null` in both exports and the only trigger is a webhook (`startTrigger`), with `isEnabled: false`. Whatever calls that webhook is outside Retool and must be found before porting, or the port loses the thing that fires it. Both are the same 5-block graph — one SQL count against `"Challenge"` (`Scheduled`+`System` within 48h for *Not Prepared*, `Active`+`System` within the last 24h for *Not Started*), a branch on the count being zero, then a Discord role ping and a PagerDuty `trigger`. Only the PagerDuty block hangs off the branch; the Discord block has no incoming edge in the export, so confirm against the live workflow whether both paths are meant to be conditional. Raw exports committed under retool-exports/raw/, **sanitized before their only commit** — verified 2026-08-10: the Discord webhook, the PagerDuty routing key and the `run-jobs` token are all `<REDACTED>` in every version in history, so **nothing here needs rotating**. The live values exist only in the ClickUp attachments. Re-sanitize the same three if the exports are ever refreshed. |
