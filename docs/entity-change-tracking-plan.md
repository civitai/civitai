...
| --- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `upsertModel` update branch — `model.service.ts:2536`                                              | **Already reads `beforeUpdate`** (`:2349`) with name/description/poi/minor/sfwOnly/nsfw/lockedProperties | Widen select with `allowNoCredit`, `allowCommercialUse`, `allowDerivatives`, `allowDifferentLicense`, `availability`, `mode`; diff `data` against it around the `tx.model.update` at `:2545` — that write is inside `upsertModel`'s `dbWrite.$transaction`, so emit **after** the transaction resolves, not in the callback (`no-io-in-transaction`) |
