...
| `src/server/common/moderation-helpers.ts` | **Contract.** `unpublishReasons`/`articleUnpublishReasons` codes written by moderator, read/displayed by main; `legacyArticleUnpublishReasons` is read-only copy for keys the picker retired. Mis-filed under `server/common/`. | **none** (pure) |
