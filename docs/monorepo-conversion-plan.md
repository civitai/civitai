...
Supersedes [civitai-schema-common-plan.md](./civitai-schema-common-plan.md) (submodule approach) for the *future-state* model. The schema-common findings still apply — what was going to be a submodule is now a workspace package.
...
├── prisma/                   # MOVES to packages/civitai-schema-common
├── packages/
│   ├── civitai-schema-common/
│   ├── civitai-db/
│   ├── civitai-redis/
│   ├── civitai-clickhouse/
│   ├── civitai-axiom/
...
### Phase 1: `@civitai/schema-common` (foundation)

Contains the Prisma schema + generated client + cross-cutting enum/constant contracts.

**Move:**
- `prisma/schema.full.prisma` → `packages/civitai-schema-common/prisma/schema.full.prisma`
- `prisma/schema.prisma` (auto-generated slim) → same package
- `prisma/migrations/` → same package
- `prisma/programmability/` → same package
- `prisma/seed.ts` → same package
- `src/shared/utils/prisma/enums.ts` → `packages/civitai-schema-common/src/enums.ts`
- `src/shared/utils/prisma/models.ts` → same
- `src/shared/utils/flags.ts` + `src/shared/constants/browsingLevel.constants.ts` + `model-version-flags.constants.ts` + `user-flags.constants.ts` → same (these are DB-encoded bit values)
- `src/shared/utils/air.ts` → same
...
```
The client now lives *inside* the package, not in root `node_modules/.prisma/client`. Both apps import from `@civitai/schema-common/client`.
...
**Update main app imports:** find/replace
- `@prisma/client` → `@civitai/schema-common/client`
- `~/shared/utils/prisma/enums` → `@civitai/schema-common/enums`
- `~/shared/utils/flags` → `@civitai/schema-common/flags`

Or — much safer for the first phase — leave a re-export shim in the old locations:
```typescript
// src/shared/utils/prisma/enums.ts
export * from '@civitai/schema-common/enums';
```
Old imports keep working; migrate them away gradually.

### Phase 2: `@civitai/db` (Postgres)

**Move:**
- `src/server/db/client.ts` (Prisma client wrapper)
- `src/server/db/db-helpers.ts` (553 lines — the labeled pg pools, `cancellableQuery`, prom-client histogram registration)
- `src/server/db/pgDb.ts`, `notifDb.ts`, `datapacketDb.ts`
- `src/server/db/db-lag-helpers.ts`
...
- `Prisma.Sql` type imports come from `@prisma/client`, which now resolves to `@civitai/schema-common/client`. Same runtime, same types — just a different module path.

### Phase 3: `@civitai/redis`
...
The `fail-open-log.ts` currently uses `logToAxiom` directly. Inject the logger via config so the redis package doesn't hard-depend on `@civitai/axiom` (keeps dependency graph one-way: app composes packages, packages don't compose each other).

**Cache key constants:** the *keys* (string prefixes, TTLs) might be better placed in `@civitai/schema-common` since they're contracts both apps must agree on. The *cache implementations* (`createCachedObject(...)` calls) stay in this package. Split: `@civitai/schema-common/redis-keys` exports the constants, `@civitai/redis` exports the implementations that use them.

### Phase 4: `@civitai/clickhouse`
...
`prisma/migrations/` moves to `packages/civitai-schema-common/prisma/migrations/`. The migration scripts in `scripts/` (e.g., `prisma-migrate-with-views-workaround.mjs`) update their paths. Manual application convention (per CLAUDE.md) doesn't change.

### Prisma client version pinning

`@prisma/client` and `prisma` (CLI) move to `packages/civitai-schema-common/package.json`. Main app no longer declares them directly — depends on `@civitai/schema-common` which depends on `@prisma/client`. Single Prisma version across the workspace.
...
2. **Updated Prisma schema paths.** Current Dockerfile has two `COPY` lines that pin `prisma/schema.full.prisma` and `prisma/schema.prisma` for layer caching — update both to `packages/civitai-schema-common/prisma/...`.
...
@ai: Clarified — "re-export shim" means leaving the old file (e.g. `src/shared/utils/prisma/enums.ts`) as a one-line file that re-exports from the new package: `export * from '@civitai/schema-common/enums'`. Existing imports `~/shared/utils/prisma/enums` continue compiling unchanged. **Drift-tolerant** = keep these shim files indefinitely so we never need to mass-rewrite the ~2,500 call sites in main app. Going with drift-tolerant. Cost is having two valid import paths for the same code (some convention noise in reviews); benefit is zero forced churn.

5. **Move main app to `apps/main/` eventually?**
@dev: I think we would move it to apps/main/ eventually.
@dev (update): Skipping Phase 7 for now to avoid needing a freeze week. New apps live in `apps/`; main app stays at root indefinitely. Revisit only if a third app or strong reason emerges.
@ai: Acknowledged. Phase 7 is now genuinely optional — the shim approach and factory-pattern packages don't depend on it. Main-at-root is fully supported by pnpm workspaces as a first-class member (`packages: ['.', 'packages/*', 'apps/*']`). The asymmetric layout has no functional cost; the only friction is mild aesthetic inconsistency if a third app eventually joins.

### Phase 7 (optional, not planned): move main app to `apps/main/`
...
| 1 | `@civitai/schema-common` | High | Medium — Prisma client path change is the trickiest single step |
| 2 | `@civitai/db` | Medium | Medium — circular dep to untangle |
| 3 | `@civitai/redis` | Medium | Low — lots of files, mostly mechanical |
| 4 | `@civitai/clickhouse` | Low | Low |
| 5 | `@civitai/axiom` | Low | Low |
| 6 | `@civitai/telemetry` | Medium | Medium — splitting auto-instrumentation from helpers requires care |
| 7 | Move main app to `apps/main/` | — | **Not planned** — kept at root indefinitely to avoid freeze-week cost |

After Phase 6, the foundation is in place for `apps/moderator` to exist as a workspace member that consumes these packages — that's a separate plan (see [moderator-app-shared-modules.md](./moderator-app-shared-modules.md)).
