...
A different registrable domain can't see the `.civitai.com` cookie. So the hub issues a short-lived
**authorization code** (the OAuth auth-code + PKCE first-party bridge), redirects to the spoke with it, and the
spoke **redeems** it server-side at the hub (`POST /api/auth/oauth/session`) for its own `civ-token` (stored in
