...
- 🚧 **Pricing templates** — creator-authored fee + paid-access defaults targeting model type × base model/ecosystem, pre-filling the main-app version form. Plan: [pricing-templates.md](pricing-templates.md). Resolves **#17**; needs no product decision (see the B9 note there). A per-type localStorage stopgap already ships in the main app and is retired by phase 4 of that plan. **Blocked on paid-access unification** — templates store a gate, and its shape is changing.
- 🟢 **Paid access: discounts + a free-unlock pledge** — reconcile early access and permanent paid access into one system, with a scheduled discount ladder and an irrevocable free-unlock pledge. Plan: [paid-access-decay.md](paid-access-decay.md) — **design settled 2026-09-02**, all 17 product questions answered, nothing schema-shaped left open. Four non-blocking follow-ups (copy, notification cadence, a resolution rule) are listed there. Blocks pricing templates until built.
...
- [x] **Lifetime "total comments received" stat** — DONE: `getAllTimeTotals` surfaces all-time reactions + comments as a context line on `/analytics` (the one place comments appear — no fast period-scoped source). Comments come from Postgres (`Thread.commentCount` over the image's thread and its replies); the `image_metrics_user` rollup it originally read is dead and returned **0** for every creator newer than userId 9.66M. ClickHouse has no correct substitute — see the note on `getAllTimeTotals` before reaching for the `comments` table, whose `entityId` is a comment id, not an image id.
