...
| B14 | Licensing-fee lineage: single boundary vs stacked ancestors | **OPEN — awaiting Justin (2026-07-14). Blocks proceeding on the earnings/fee-resolution work.** When a model derives from an ancestor licensing root, the current code charges the **nearest root's fee only** — one lineage (`baseModel`-role) fee to that root's owner — **plus** the creator's own version surcharge stacked on top (paid to them). It does **not** charge multiple ancestors up the chain: building on Turbo charges Turbo's rate, "not the base's." (`licensingSourceVersionId` is enforced to point at a `LicensingRoot` — `model-version.controller.ts:437`; resolution + stacking in the mini endpoint `api/v1/model-versions/mini/[id].ts`.) **Question:** is single-boundary correct, or should **every** ancestor licensor in the chain earn a cut (transitive stacking)? Recommended default: **single-boundary** (current behavior) unless product wants transitive. | 🟢 |
