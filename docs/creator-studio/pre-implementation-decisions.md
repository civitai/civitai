...
| B9 | Default fee suggestions | **No default fee** — off unless a creator turns one on. When they do, seed the input: **LoRA ~0.1**, **base/checkpoint ~1** buzz/image; nothing for other types. **Scope (2026-08-31):** this governs *Civitai's* suggested values and the rule that nothing is priced without creator action. It does **not** speak to creator-authored defaults — #17 read it as forbidding those, which it never said. See [pricing-templates.md](monetization/pricing-templates.md). | ✅ |
