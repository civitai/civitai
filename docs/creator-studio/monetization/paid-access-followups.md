...
- [ ] **Update Justin's article to the monthly pricing allowance: Free 3 / Bronze 10 / Silver 25 / Gold unlimited per month.** These replace the old concurrent permanent-gate caps, which no longer exist — the numbers look similar and mean something different, so the article has to say "new prices per calendar month", not "models on paid access". See R3 in [../features/monetization-rules.md](../../features/monetization-rules.md). _(Justin ~16:47)_
