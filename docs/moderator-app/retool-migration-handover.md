...
| 7 | **`RatingChanges`** — **half done 2026-08-21.** `LogNsfwLevel` (the rating trail) is built and unexercised. `LogNsfwLevel2` (the tag-vote write) is blocked on one fact the export description does not carry: what its NOT NULL `originalRating` holds. | Canonical state: [Front Page Audit port state](retool-exports/parity-findings.md) |
