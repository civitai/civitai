...
| ~~2~~ | ~~Set **`RETOOL_DATABASE_URL`** in every deployed env~~ **RESOLVED 2026-08-21 — do NOT provision this.** The Retool and moderator databases were consolidated into one, so `MODERATOR_DATABASE_URL` is the only name and `getModeratorDb()` reads it. Confirm THAT variable is set in every deployed env. | — |
...
| 4 | Apply **5 SQL migrations** by hand (four are `CREATE INDEX CONCURRENTLY`, so each runs outside a transaction) | Page-access table + mod-activity history + a report index + Bulk Ban's email-domain index (**which needs `ANALYZE "User";` after it or the planner ignores it**) + the two comment `userId` indexes the ban purge needs. The last two are applied to production already, nowhere else → [detail](retool-migration-handover-detail.md#2-database-migrations--none-are-auto-applied) |
| 5 | **Grant 7 new pages on `/admin`** — `article-lookup`, `user-reports`, `post-reports`, `bulk-image-manager`, `front-page-audit`, `image-help`, `users/newest` | A new page is reachable only by `moderator:admin` until granted. Grant Bulk Image Manager **narrowly**. Front Page Audit's rating buttons are gated on its own grant, so a moderator without it sees the sweep read-only. 🔴 Grant `users/newest`, never `/users` — the latter is also what User Lookup's ban, purge and bulk-comment actions check → [detail](retool-migration-handover-detail.md#2b-grant-the-new-pages-on-admin) |
...
| 12 | **Run the legacy-strike migration** — `pnpm exec tsx --env-file=apps/moderator/.env apps/moderator/moderator-db/migrate-legacy-strikes.ts`, dry by default, then `--apply` | **No ordering requirement — before or after the deploy, per environment.** It writes only existing columns and enum values, and the display subtracts what it has copied, so history is never shown twice and never missing. Imported rows are Expired/zero-point and cannot escalate; the verification pass fails the run if any is countable → [detail](legacy-strike-migration.md) |
...
| ~~7~~ | ~~**`RatingChanges`** — the one Front Page Audit write still unported.~~ **RESOLVED 2026-08-21.** Both writes built from the app export, which also corrected two facts the audit notes had wrong (see the canonical row). Unexercised against a database. | Canonical state: [Front Page Audit port state](retool-exports/parity-findings.md) |
...
| ~~10~~ | ~~**Two strike systems** — this app writes Retool's `UserStrikes`.~~ **RESOLVED 2026-08-20** (write) **and 2026-08-21** (data). `issueStrike` writes the main app's `UserStrike`; the ~12.9k legacy `UserStrikes` rows now have a migration into `UserStrike` as inert history. It is written and **unapplied** — see blocker 12. | [detail](legacy-strike-migration.md) |
