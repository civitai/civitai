...
| ~~12~~ | ~~**Run the legacy-strike migration**~~ **RESOLVED for production 2026-08-21** — 12,902 rows imported; an earlier import pass's duplicates were deleted 2026-08-24. **Preview is still unrun**: `pnpm exec tsx --env-file=apps/moderator/.env apps/moderator/moderator-db/migrate-legacy-strikes.ts`, dry by default, then `--apply`. | No ordering requirement — before or after the deploy, per environment → [detail](legacy-strike-migration.md) |
...
| ~~10~~ | ~~**Two strike systems** — this app writes Retool's `UserStrikes`.~~ **RESOLVED 2026-08-20** (write) **and 2026-08-21** (data). `issueStrike` writes the main app's `UserStrike`; the ~12.9k legacy `UserStrikes` rows now have a migration into `UserStrike` as inert history. Applied to production 2026-08-21, duplicates from an earlier pass cleaned up 2026-08-24; preview outstanding — see blocker 12. | [detail](legacy-strike-migration.md) |
