...
## Operations
- The `userSelectable` column migration is **applied manually per environment** (repo convention). Ship the committed migration + the `UPDATE ... WHERE name IN ('CivBot','CivChan')` seed, and surface both for preview/staging/prod apply. Until applied, the whitelist fallback keeps the current two judges working.
