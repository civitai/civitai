...
- **Paid-out challenges are delete-guarded** — `ChallengeWinner` is ON DELETE CASCADE, so deleting a `Completed` challenge erases who won while the prize Buzz stays spent in the ledger and winners keep their cosmetics — a badge with no surviving explanation. `Completing` is mid-payout and worse. `deleteChallenge` refuses both unless called with `{ force: true }`; neither status is voidable (`voidChallenge` takes Active/Scheduled/Cancelled only), so a hard block would strand moderators on a challenge that has to come down. No UI passes `force` — `challenge.delete` accepts it on `moderatorDeleteChallengeSchema` and both call sites send `{ id }` alone. `Active` stays refused outright, force or not. Creators never reach this: `deleteUserChallenge` already allows only `Scheduled` (0 entries) or `Cancelled`.
- **Entrant posts survive deletion** — `Post.collectionId` was ON DELETE CASCADE and `Image.postId` is ON DELETE SET NULL, so dropping a challenge's collection deleted the submission post of everyone who entered and left their images alive with no post (a 404 on the owner's own image, still served from the feed index, which requires `postId IS NOT NULL` at build time only). `Post.collectionId` is a back-reference recording where a post was created, not ownership — `CollectionItem` is the membership join and keeps its CASCADE. Migration `20260901000000_post_collection_set_null` flips the FK to SET NULL, and that is what makes every path safe, including the ones no service owns (raw SQL, Retool, the `User` -> `Collection` cascade). **It is applied by hand and is not automatic** — an environment it has not reached still cascades. `deleteChallenge` and `deleteCollectionById` still call `detachPostsFromCollection` (`src/server/services/collection-post-detach.ts`) first, not for correctness but for cost: the FK's own SET NULL measures 23.0s on the largest contest collection (22,833 posts), holding row locks on all of them, and in `deleteChallenge` it would also blow past Prisma's 5s interactive transaction default — which is why that call sits outside the transaction. Account deletion relies on the FK alone. A partial detach is safe to retry. Entries still go with the collection; the posts become ordinary posts.
...
Creators can **view / edit their own `User` challenges only while `Scheduled` with 0 entries**. Delete additionally allows `Cancelled` — a moderator-voided challenge is already dead and refunded, so the entry-count block guards `Scheduled` only and a Cancelled challenge is deletable however many entries it holds. End/void/pick-winners stay moderator-only.
- `deleteUserChallenge({ id, userId })` — `protectedProcedure`, both flags; guards owner + `source=User` + (`Scheduled` with 0 entries, or `Cancelled`), then `deleteChallenge` (refunds escrowed prize, cascades — entrant posts detached first, see Safety / gating). Delete re-reads status so it **fails safe** if it races activation (blocks on `Active`); full idempotency under concurrent delete is an open verification item **(deferred)**.
- `getUserChallengeForEdit({ id, userId })` — owner-or-moderator-gated edit payload. **Not** status-filtered: a live or finished challenge still opens, read-only (the form disables every field past `Scheduled`), so the write gate in `upsertUserChallenge` is the only editability check.
