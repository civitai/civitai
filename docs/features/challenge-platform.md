...
- **Entrant posts survive deletion** — `Post.collectionId` is ON DELETE CASCADE and `Image.postId` is ON DELETE SET NULL, so dropping a challenge's collection would delete the submission post of everyone who entered and leave their images alive with no post (a 404 on the owner's own image, still served from the feed index, which requires `postId IS NOT NULL` at build time only). `deleteChallenge` calls `detachPostsFromCollection` (`src/server/services/collection-post-detach.ts`) first, and deliberately **outside** the delete transaction: the largest collection measures ~25s, past Prisma's interactive default. A partial detach is safe to retry. Entries (`CollectionItem`) still go with the collection; the posts become ordinary posts. `deleteCollectionById` and account deletion (`removeAllContent`, via `detachPostsFromUserCollections`) do the same — a contest collection holds other people's posts, not its owner's.
