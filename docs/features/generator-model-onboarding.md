...
- **Coverage.** The first branch of the `GenerationCoverageNext` view — what the app reads, via the Prisma `GenerationCoverage` model's `@@map` — covers any version listed in `"EcosystemCheckpoints"`, and it does **not** check status. A top-level `m.mode IS NULL` still applies, so an `Archived` or `TakenDown` model is covered by no branch whatever rows it has. The view is a plain view, so the row takes effect as soon as it is inserted. `resourceDataCache` sits in front of it with a one-hour TTL, so bust the cache with `modelVersion.bustCache` after inserting.
