...
- **Coverage.** The `"EcosystemCheckpoints"` branch of the `GenerationCoverage` view — the one view the app reads, which carries both rules as columns and covers via this branch under **both** — covers any version listed in that table, and it does **not** check status. A top-level `m.mode IS NULL` still applies, so an `Archived` or `TakenDown` model is covered by no branch whatever rows it has. The view is a plain view, so the row takes effect as soon as it is inserted. `resourceDataCache` sits in front of it with a one-hour TTL, so bust the cache with `modelVersion.bustCache` after inserting.
