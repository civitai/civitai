 ...
 *
 * 🔴 It gets its OWN module, rather than living inside `missing-media.ts` with its only current
 * caller, because it was previously open-coded twice — by OPPOSITE construction — and the two
 * answers disagreed on real rows. One spelling asked "does it carry a URI scheme?" and probed
 * everything else; the other asked "is it a bare uuid?" and probed only that. For `some-file.png`
 * the first probes, gets a 404, and PERMANENTLY refuses a moderator publish; the second never asks.
 * A predicate open-coded at N sites regenerates the same bug at all N, so there is one copy and
 * every existence check imports it — the two `resolveIngestionError` guards today, and any
 * upload-time check added later.
 *
