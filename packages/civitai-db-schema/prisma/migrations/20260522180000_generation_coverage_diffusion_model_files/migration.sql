-- Extend the GenerationCoverage file-type allowlist to include 'Diffusion Model'
-- and 'UNet'. Modern ecosystem checkpoints (Flux, Wan, ZImage, Anima, Ernie, etc.)
-- upload their main weights under these newer type names instead of the legacy
-- 'Model' / 'Pruned Model'. The orchestrator treats Diffusion Model files as
-- equivalent to Checkpoint Model files for generation, so they belong in the
-- "version has scanned primary weights" check.
