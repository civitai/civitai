...
FROM (
	SELECT
		...
		ROW_NUMBER() OVER (PARTITION BY mm.timeframe ORDER BY mm."downloadCount" DESC, m.Id DESC) AS "downloadCountRank",
		COALESCE(mm."ratingCount", 0) AS "ratingCount",
		ROW_NUMBER() OVER (PARTITION BY mm.timeframe ORDER BY mm."ratingCount" DESC, m.Id DESC) AS "ratingCountRank",
		COALESCE(mm."rating", 0) AS "rating",
		ROW_NUMBER() OVER (PARTITION BY mm.timeframe ORDER BY mm.rating DESC, m.Id DESC) AS "ratingRank",
		tf.timeframe
	FROM "Model" m
	CROSS JOIN (
