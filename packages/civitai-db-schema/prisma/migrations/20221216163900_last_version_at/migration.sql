-- AlterTable
ALTER TABLE "Model" ADD COLUMN     "lastVersionAt" TIMESTAMP(3);

-- Add lastVersionAt value
WITH mv AS (
  SELECT "modelId", MAX("createdAt") "createdAt" FROM "ModelVersion" GROUP BY "modelId"
)
UPDATE "Model" m SET "lastVersionAt"=mv."createdAt"
FROM mv
...
AS
  SELECT
	...
	MAX(IIF(t.timeframe = 'AllTime'::"MetricTimeframe", t."newRank", NULL::bigint)) AS "newRank"
FROM (
	SELECT
		m.id AS "modelId",
		...
		ROW_NUMBER() OVER (ORDER BY COALESCE(m."lastVersionAt", m."createdAt") DESC, m.Id DESC) AS "newRank",
		tf.timeframe
	FROM "Model" m
	CROSS JOIN (
