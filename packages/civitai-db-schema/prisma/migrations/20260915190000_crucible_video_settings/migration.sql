...
--
-- 🔴 APPLY THIS BEFORE THE CODE DEPLOYS, not after. Both columns join `crucibleDetailSelect`, so
-- the new build SELECTs them on every crucible read — including image crucibles, which never use
-- them. Against a database without the columns that is not a degraded feature, it is a Prisma
-- error on every crucible detail and judge request. The reverse order is safe: the columns are
-- nullable and nothing reads them until the deploy lands.
