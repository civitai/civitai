...
The shadcn primitives under `src/lib/components/ui/` are CLI-generated and regenerated by `--overwrite`
above, so never hand-edit them. Hand-written components go in a sibling directory:
`src/lib/components/selection/` (`SelectionCheckbox`). Also `utils.ts` (`cn` + type helpers), `theme.css`,
and two hooks: `is-mobile` and `selection-set` (`SelectionSet`, a `SvelteSet` with Gmail-style shift-click
range selection). Civitai-specific composites (EdgeMedia, ImageGuard, masonry, moderation toolbars) belong
here too over time — built on these primitives. See [docs/packages/new-app-integration.md](../../docs/packages/new-app-integration.md) §0.

Tests: `pnpm --filter @civitai/ui test` (vitest, `src/**/*.test.ts`); CI runs them in `Package unit tests`.
