...
describe('ClusterSelfHealWatchdog re-arms after a heal (FIX #2, real counter)', () => {
  const CFG: ClusterSelfHealConfig = {
    ...
    sustainedMs: 20000,
    cooldownMs: 60000,
    // Deadline trigger disabled (<= 0) so this suite exercises ONLY the inflight path; no
    // jitter so the reconnect fires synchronously and the assertions stay deterministic.
