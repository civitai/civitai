...
PrismaGenHelper.generatorHandler({
  ...
  async onGenerate(options) {
    ...
    // Formatted here, not left raw: the committed copy is prettier-wrapped, so a generator emitting
    // unwrapped type aliases makes `db:check-generated` fail on a clean checkout and leaves the file
    // dirty after every `pnpm install` (postinstall runs db:generate).
    const prettierConfig = await prettier.resolveConfig(outputPath);
    const source = prettier.format(header + output.join('\n'), {
      ...prettierConfig,
      parser: 'typescript',
    });
