...
const TARGETS = {
  ...
  // `withSysReadDeadline` is a FLAT root, not a client: the canonical mock exposes it as a
  // single node whose default is the real wrapper, so a factory replacing it converts rather
  // than being refused as a control surface. It became canonical at 17f994221e; before that
  // the refusal was correct, and nine files across three slices were still being refused on
  // a description of the world before the seam existed.
  '~/server/redis/client': {
    roots: ['redis', 'sysRedis', 'withSysReadDeadline'],
    flatRoots: ['withSysReadDeadline'],
    mock: 'redisMock',
...
function convert(file, text) {
  ...
  for (const stmt of sf.statements) {
    ...
    for (const prop of obj.properties) {
      ...
      if (spec.flat || spec.flatRoots?.includes(root)) {
        if (
          !collect(
            prop.initializer,
            `${spec.mock}.${root}`,
