...
async function main() {
  const files = await fs.readdir('./packages/civitai-db-schema/prisma/programmability');

  const operations = [];
  for (const file of files) {
    const content = await fs.readFile(`./packages/civitai-db-schema/prisma/programmability/${file}`, 'utf-8');
