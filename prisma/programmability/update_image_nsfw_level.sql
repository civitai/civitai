CREATE OR REPLACE FUNCTION update_image_nsfw_levels(image_ids INTEGER[])
RETURNS VOID AS $$
BEGIN
  WITH image_level AS (
    SELECT
...
$$ LANGUAGE plpgsql;
---
CREATE OR REPLACE FUNCTION update_image_nsfw_level(VARIADIC image_ids INTEGER[])
RETURNS VOID AS $$
BEGIN
	PERFORM update_image_nsfw_levels(image_ids);
END;
$$ LANGUAGE plpgsql;
