-- AlterTable
ALTER TABLE "Model" ADD COLUMN     "publishedAt" TIMESTAMP(3);

-- Alter View
CREATE OR REPLACE VIEW "ModelRank"
AS
  SELECT
	t."modelId",
...
FROM (
	SELECT
		...
		ROW_NUMBER() OVER (ORDER BY COALESCE(mv."createdAt", m."createdAt") DESC, m.Id DESC) AS "newRank",
		tf.timeframe
	FROM "Model" m
	CROSS JOIN (
	...
	LEFT JOIN LATERAL (
	  SELECT MAX("createdAt") "createdAt" FROM "ModelVersion" WHERE "modelId" = m.id
	) mv ON TRUE
) t
GROUP BY t."modelId";