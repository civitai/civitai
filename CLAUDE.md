...
Several repo conventions are enforced by tests, not by eslint. 36 live in `src/server/services/__tests__/no-*.test.ts` — `no-agent-ground-truth-write`, `no-coerce-boolean-in-api`,
||||||| 1216d71d16
Several repo conventions are enforced by tests, not by eslint. 34 live in
