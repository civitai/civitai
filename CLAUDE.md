...
Several repo conventions are enforced by tests, not by eslint. 35 live in `src/server/services/__tests__/no-*.test.ts` — `no-agent-ground-truth-write`, `no-coerce-boolean-in-api`,
...
`test:lint-rules` names 40 files today.
