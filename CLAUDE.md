...
Several repo conventions are enforced by tests, not by eslint. 25 live in
...
`no-throwing-image-query-parse` (a bare `.parse` of `imagesQueryParamSchema` — one junk query param
threw in a page render and returned a 500 on links the app had mailed its own users),
