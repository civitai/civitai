...
### Core Technologies
- **Framework**: Next.js 16 with TypeScript (Node >= 22; `.nvmrc` pins the version)
...
- **DB** — `db/db-helpers.ts` (raw pg-pool config: `connectionTimeoutMillis`, labeled pool gauges), Prisma client; schema `packages/civitai-db-schema/prisma/schema.full.prisma` (the `schema.prisma` beside it is generated by `pnpm db:generate` — editing it is discarded). **Migrations are applied manually — see the Database rule above.**
