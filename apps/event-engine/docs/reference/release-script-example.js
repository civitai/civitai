...
/**
 * Release Script for simple-journal
 *
...
// Helper to update a single package.json file
function updateSinglePackageJson(relativePath, newVersion) {
...
function updatePackageVersion(newVersion) {
  // @meta: Refactored to DRY up the repetitive code
  ...
  packagesToUpdate.forEach(packagePath => {
    updateSinglePackageJson(packagePath, newVersion);
    log.success(`Updated ${packagePath} version to ${newVersion}`);
  });
}
...
function commitVersionBump(version) {
  ...
  gitCommand('add package.json frontend/package.json backend/package.json', { silent: true });
...
function showSummary(version) {
  ...
  console.log(`  Tag:            ${chalk.green(`v${version}`)}`);
  console.log(`  Main branch:    ${chalk.green(mainCommit)}`);
  console.log(`  Release branch: ${chalk.green(releaseCommit)}`);
  console.log(`  Remote:         ${chalk.green(REMOTE)}`);
  console.log('');
  console.log(chalk.yellow('Next steps:'));
  console.log('  • Verify deployment in staging/production environment');
  console.log('  • Monitor application logs for any issues');
  console.log('  • Run post-deployment tests if available');
  console.log('');
  console.log(chalk.blue('Pre-release checks completed:'));
...
// Show help text
function showHelp() {
  console.log('Release Script for simple-journal');
  console.log('');
