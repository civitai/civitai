I don't think we ever use the create cached array version of this, really. So really, we only need the create cached object version of this as a reference. But obviously, because it uses the create cached array, we'll have to think about how to reimplement this as just a single thing.

Full source: C:\Dev\Repos\work\model-share\src\server\utils\cache-helpers.ts
Use this to find other references such as redis.packed. Note that we will likely want to create our own wrapper that creates an IRedis instance that does what packed does (writing/reading things as binary with msgpackr). That way we can have anyone pass in a standard IRedis instance and we can upgrade it to a instance that will use msgpackr. The helper should accept a packr read/write fn so it doesn't have a direct dep.
