...
function loadEnv() {
  if (!existsSync(ENV_PATH)) {
    console.error('Error: .env file not found in skill directory');
    console.error('');
    console.error('Setup:');
    console.error('  1. Copy .env-example to .env in the skill directory');
    console.error('  2. Add your ClickHouse credentials');
    process.exit(1);
  }
...
if (!writable) {
  const writeOps = ['INSERT', 'DELETE', 'DROP', 'ALTER', 'TRUNCATE', 'CREATE', 'RENAME', 'OPTIMIZE', 'GRANT', 'REVOKE'];
...
// Check if this is a DDL/DCL statement that doesn't return rows
const ddlOps = ['GRANT', 'REVOKE', 'CREATE', 'DROP', 'ALTER', 'TRUNCATE', 'RENAME', 'OPTIMIZE', 'SET', 'USE', 'KILL'];
const isDdl = ddlOps.some(op => upperQuery.startsWith(op));
...
async function main() {
  ...
  try {
    ...
    // Use command() for DDL/DCL statements that don't return rows
    if (isDdl) {
