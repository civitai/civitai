<script lang="ts">
  ...
  import { chatAuditChatUrl, chatAuditUserUrl } from '$lib/entity-url';
  import type { ModeratorContact } from '$lib/server/user-signals.service';

  let {
    modContact,
    username,
  }: { modContact: ModeratorContact; username: string | null } = $props();

  const postedInHref = $derived(username ? chatAuditUserUrl(username) : null);
</script>
...
<section class="mb-4 rounded-xl border border-dark-4 bg-dark-6 p-5">
  ...
  {#if postedInHref}
    <p class="mb-3 text-sm">
      <!-- "posted in", not "is in" — see `chatAuditUserUrl`. -->
      <a href={postedInHref} class={LINK_CLASS}>Every chat this account has posted in →</a>
    </p>
  {/if}
  ...
  {:else if modContact.chats === 0}
    <p class="text-sm text-dark-2">
      No moderator has posted in a chat with this account. That is not the same as having no chats.
    </p>
  {:else}
    ...
    <ul class="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm">
      {#each modContact.chatIds as id (id)}
        <li>
          <a href={chatAuditChatUrl(id)} class={LINK_CLASS}>chat {id}</a>
        </li>
      {/each}
