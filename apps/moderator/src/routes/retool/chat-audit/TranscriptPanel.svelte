...
<section class="mb-4 grid gap-4 lg:grid-cols-[2fr_1fr]">
  <div class="rounded-xl border border-dark-4 bg-dark-6 p-5">
    <h3 class="mb-1 text-sm font-semibold text-white">
      Transcript — chat {chatId} ({transcript.rows.length}{transcript.truncated ? '+' : ''})
    </h3>
    <p class="mb-3 text-xs text-dark-2">Private messages. Oldest first.</p>
        ...
        {#each transcript.rows as m (m.id)}
          <li>
            <div class="flex flex-wrap items-baseline gap-x-2">
              <a href="/retool/user-lookup?q={m.userId}" class={LINK_CLASS}>
                {m.username ?? `#${m.userId}`}
              </a>
              {#if m.bannedAt}<Badge variant="destructive">banned</Badge>{/if}
              <span class="text-xs text-dark-2">{dateTime(m.createdAt)}</span>
            </div>
            <p class="whitespace-pre-wrap wrap-break-word text-dark-0">{m.content}</p>
          </li>
        {/each}
      </ul>
    {/if}
