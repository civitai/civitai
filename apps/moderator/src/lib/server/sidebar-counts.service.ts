...
// tag, newUser, modRule, remixSource) plus `imageTags`, `imageRatings`, `appeals`, `reported`, and the
// article queues (`articles` = unpublished, `articleRatings` = pending disputes). A badge should mean "this
// much needs action", so slow backlogs (ingestion errors ~50k, images-to-ingest) get none. Most are cheap +
// index-backed; `reported` is ~270ms (rides the Report(status) partial index) but streamed, so it never
// blocks render.
  ...
  const [modes, imageTags, imageRatings, appeals, reported, articles, articleRatings] =
    await Promise.all([
      ...
      dbRead
        ...
        .executeTakeFirst(),
      // The image-rating-review queue: pending ImageRatingRequest disagreements (weight >= 3).
      ...
      dbRead
        ...
        .executeTakeFirst(),
      // Unpublished articles awaiting a decision, and pending article rating disputes.
