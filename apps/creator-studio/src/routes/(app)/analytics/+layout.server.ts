...
import { rollupThrough } from '$lib/date-range';
...
export const load: LayoutServerLoad = ({ cookies }) => {
  ...
  // `image_views_daily_by_owner` is a nightly rollup, so its series ends a day before the others.
  const viewsThrough = rollupThrough(through);
  return {
    range,
    through,
    viewsThrough,
